...
def load_dll(path_dll=None):
    if not path_dll:
        ...
        subpath_dll = r'/LGHUB/sdk_legacy_led_x64.dll'.format(bitness)
        try:
...
def logi_led_flash_single_key(key_name, red_percentage, green_percentage, blue_percentage, ms_duration, ms_interval):
    ...
    if led_dll:
        ...
        ms_interval = ctypes.c_int(ms_interval)
        return bool(
            led_dll.LogiLedFlashSingleKey(key_name, red_percentage, green_percentage, blue_percentage, ms_duration,
                                          ms_interval))
    else:
        return False
...
def logi_led_pulse_single_key(key_name, red_percentage_start, green_percentage_start, blue_percentage_start,
    ...
    if led_dll:
        ...
        return bool(
            led_dll.LogiLedPulseSingleKey(key_name, red_percentage_start, green_percentage_start, blue_percentage_start,
...
def logi_led_get_config_option_color(key, *args):
    ...
    if led_dll:
        ...
        if led_dll.LogiGetConfigOptionColor(key, ctypes.pointer(red), ctypes.pointer(green), ctypes.pointer(blue),
